...
#include <asm/arch/pxa1936.h>

#ifdef CONFIG_PXA_AMP_SUPPORT

#define PMU_CC2_AP	(PXA1936_APMU_BASE + 0x100)

#define PXA1936_CPU_POR_RST(n)  (1 << ((n) * 3 + 16))
#define PXA1936_CPU_CORE_RST(n) (1 << ((n) * 3 + 17))
#define PXA1936_CPU_DBG_RST(n)  (1 << ((n) * 3 + 18))

int lpm_method = LPM_PSCI;
static int do_psci_mcpm(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	if (strcmp("mcpm", argv[1]) == 0)
		lpm_method = LPM_MCPM;
	else if (strcmp("psci", argv[1]) == 0)
		lpm_method = LPM_PSCI;
...
U_BOOT_CMD(
	lpmch, 2, 1, do_psci_mcpm,
	"Choose lpm method use mcpm or psci",
	"Usage:\nlpmch [mcpm/psci]"
);
...
void pxa_cpu_reset(u64 cpu, u64 addr)
{
	...
	cpu_mpidr = CORE_ID[cpu];
	if (lpm_method == LPM_MCPM) {
		invoke_fn_smc(MCPM_PN_CPU_ON, cpu_mpidr, addr, 0);

		tmp = readl(CORE_WAKEUP[cur_cpu]);
		tmp |= PXA1936_CPU_WAKEUP(cpu);
		writel(tmp, CORE_WAKEUP[cur_cpu]);
	} else /* PSCI do not need to set up wakeup, it has been done in firmware */
		invoke_fn_smc(PSCI_FN_CPU_ON, cpu_mpidr, addr, 0);
...
{
	u32 val;
	struct pxa1936cpu_registers *cpuregs =
		(struct pxa1936cpu_registers *)PXA1936_CIU_BASE;

	struct pxa1936apbc_registers *apbclkres =
		(struct pxa1936apbc_registers *)PXA1936_APBC_BASE;

#if defined(CONFIG_I2C_MV)
	struct pxa1936apbcp_registers *apbcpclkres =
		(struct pxa1936apbcp_registers *)PXA1936_APBCP_BASE;
#endif

	struct pxa1936mpmu_registers *mpmu =
		(struct pxa1936mpmu_registers *)PXA1936_MPMU_BASE;

#if defined(CONFIG_MMP_DISP) || defined(CONFIG_MV_SDHCI) || \
	defined(CONFIG_MV_UDC)
	struct pxa1936apmu_registers *apmu =
		(struct pxa1936apmu_registers *)PXA1936_APMU_BASE;
#endif
