...
void mmp_log_buf_write(const char *text)
{
	unsigned int pos, size;
	static char *mmp_log_buf = (char *)CONFIG_MV_LOG_BASE;
	static unsigned int *mmp_log_pos;
	static unsigned int *mmp_log_mag;
	static unsigned int mmp_log_size;

	if (!mmp_log_size)
		mmp_log_size = CONFIG_MV_LOG_SIZE - sizeof(*mmp_log_pos) -
			sizeof(*mmp_log_mag);
	if (!mmp_log_pos)
		mmp_log_pos = (unsigned int *)(mmp_log_buf + mmp_log_size);
	if (!mmp_log_mag)
		mmp_log_mag = (unsigned int *)(mmp_log_buf + mmp_log_size +
				sizeof(*mmp_log_pos));
	...
	if (likely(size + pos <= mmp_log_size))
		strcpy(&mmp_log_buf[pos], text);
	else {
		unsigned int first = mmp_log_size - pos;
		strncpy(&mmp_log_buf[pos], text, first);
		strcpy(&mmp_log_buf[0], text + first);
	}
