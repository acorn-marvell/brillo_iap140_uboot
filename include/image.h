...
#define IH_HEADER_SIZE	0x40		/* Image header size		*/
...
#endif
	ret = image_check_arch(hdr, IH_ARCH_DEFAULT);
#ifdef IH_ARCH_ALTERNATE
	if (!ret)
		ret = image_check_arch(hdr, IH_ARCH_ALTERNATE);
	if (ret)
		set_cpu_arch(IH_ARCH_ARM);
#endif
	return ret;
}
#endif /* USE_HOSTCC */
